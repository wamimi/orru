...
import { config, ConfigError, REPO_ROOT, WORKER_ROOT } from "./config.js"
...
/// Slips hold the only copy of the salts. The directory is therefore constrained
/// rather than taken on trust: `WORKER_SLIP_DIR=../public/slips` would otherwise
/// resolve into Next.js's public directory and serve every salary on the web.
function slipDir(): string {
  const dir = resolve(WORKER_ROOT, process.env.WORKER_SLIP_DIR ?? "slips")

  const publicDir = resolve(REPO_ROOT, "public")
  if (dir === publicDir || dir.startsWith(`${publicDir}/`)) {
    throw new ConfigError(
      `WORKER_SLIP_DIR resolves inside ${publicDir}, which Next.js serves publicly. ` +
        `Slips contain the amounts and salts the proof exists to hide.`,
    )
  }
  if (dir !== WORKER_ROOT && !dir.startsWith(`${WORKER_ROOT}/`)) {
    throw new ConfigError(`WORKER_SLIP_DIR must stay under ${WORKER_ROOT}, got ${dir}`)
  }
...
export function issueSlips(options: IssueSlipsOptions): SlipBook {
  ...
  mkdirSync(slipDir(), { recursive: true, mode: 0o700 })
  ...
  if (existsSync(path)) {
    ...
    // A random salt makes every reissue a different commitment, so a repeated
    // period would silently shadow the anchored one rather than collide.
    const clash = existing.slips.filter((e) => book.slips.some((n) => n.period === e.period))
    if (clash.length > 0) {
      throw new ConfigError(
        `slips already exist for period(s) ${clash.map((c) => c.period).join(", ")} of ` +
          `${recipient} from this payer. Reissuing would replace salts that may already be ` +
          `anchored. Choose a later --from-period, or delete ${path} deliberately.`,
      )
    }

    book.slips = [...existing.slips, ...book.slips].sort((a, b) =>
      BigInt(a.period) < BigInt(b.period) ? -1 : 1,
    )
  }

  writeAtomic(path, `${JSON.stringify(book, null, 2)}\n`)
...
/// Owner-only, flushed, then renamed into place. A partial write here would lose
/// salts that exist in exactly one location.
