...
#
# Safe to fire every hour against 4-hour periods: runPayroll() refuses to run
# twice in one period, so three of four firings are expected to skip. That
# redundancy is the point — producing a gap in the period sequence then needs
# FOUR consecutive failures instead of one, and gaps cannot be backfilled.
#
# Install:
...
if [ "$current" = "$last" ]; then
  log "skip   period $current already paid"
  exit 0
fi
...
log "run    period $current (last paid $last)"
...
log "OK     period $current  status=$status  tx=$txhash"
