...
abstract contract DeployBase is Script {
    ...
    error WrongDeploymentChain(uint256 given, uint256 expected);

    /// @dev These addresses become trust roots. Recording one under the wrong
    ///      chain is silent and expensive.
    function _requireSepolia() internal view {
        if (block.chainid != SEPOLIA) revert WrongDeploymentChain(block.chainid, SEPOLIA);
    }
...
///        PAYER_ANCHOR_ADDRESS  The anchor. Must be non-zero.
...
contract DeployDemoPayroll is DeployBase {
    function run() external returns (DemoPayroll payroll) {
        ...
        console2.log("");
        console2.log("  History accrues in wall-clock time. Settlement catches up in order,");
        console2.log("  but only while the cron runs.");
