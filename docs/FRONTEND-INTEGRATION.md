...
| **Browser proof generation** | **3–15s** (1.8s measured natively; the rest is a one-time fetch) | **yes** |
...
Measured, not estimated: bb.js in WebAssembly proves this circuit in **1.8
seconds** with 8 threads, and the resulting proof is accepted by the deployed
verifier (§10g). The browser adds the one-time WASM and SRS fetch on top.

So the honest budget is 3–15s on a cold cache and near-instant once warm. Call
`prover.prepare()` during S3 and the download happens while the user is reading
their income. Still design staged messages rather than a spinner, but this is
much less painful than this document originally assumed.
...
### 10g · Proving in the browser

**Built and verified.** A proof generated by bb.js in WebAssembly is accepted by
the deployed Solidity verifier — there is a Foundry test for it,
`contracts/test/verifier/WasmProof.t.sol`, run against a real Semuni proof.

**Measured: 1.8s** in Node with 8 threads, for the ~58,000-gate circuit. Expect
3–15s in a browser, most of it the one-time WASM and SRS fetch rather than the
proving itself. Warm it early and the perceived cost mostly disappears.

**What ships**

```
public/circuit/income_proof.json     50 KB, debug symbols stripped
src/lib/prove/index.ts               Prover class + module-level `prover`
src/lib/prove/worker.ts              the Web Worker
src/lib/prove/witness.ts             slips -> Noir inputs, with validation
src/lib/prove/types.ts               PaymentSlip, ProveInput, ProveResult
```

Dependencies are pinned in the root `package.json` and **must not be bumped**:
`@aztec/bb.js@3.0.0-nightly.20260102` and `@noir-lang/noir_js@1.0.0-beta.18`,
matching the CLI versions the deployed verifier was generated with. A different
bb.js produces a proof that fails on-chain with no useful error.

**Using it**

```tsx
import { prover, type PaymentSlip } from '@/lib/prove'

// S3: warm the worker while the user reads their income. Free, and it hides
// the WASM download entirely.
useEffect(() => { prover.prepare() }, [])

// S5
const { proof, publicInputs } = await prover.prove(
  { recipient, band, slips },
  ({ stage }) => setStage(stage),   // 'loading' | 'witness' | 'proving' | 'done'
)
```

`proof` and `publicInputs` drop straight into the §10c issue flow.

**Where the slips come from — the part you have to design**

The circuit's private witness is `{ amount, period, salt }` per cycle. Those must
reach the browser and stop there.

- **Demo Payroll** publishes them in its `PaymentMade` events, so they can be
  read from Sepolia. Public already; nothing is lost.
- **Semuni** publishes nothing. The payer holds the slips and delivers them to
  the recipient off-chain — that is what makes the commitment hiding.

For the demo, serve them from `GET /api/slips/:address`, gated on the S2 session
signature. That endpoint *is* the payer-to-recipient channel, which is legitimate:
the payer is the party that generated them. It is also the one endpoint that
handles the private witness, so it must never log its response body.

The `ProveInput` validator rejects a window that is not exactly `PERIODS` long or
not consecutive, so a bad window fails immediately rather than as an opaque
constraint failure.
